...
derive instance newtypeZSL :: Newtype ZSL _

instance showZSL :: Show ZSL where
  show = show <<< A.fromFoldable <<< map fromCharArray <<< unwrap

instance semiringZSL :: Semiring ZSL where
  zero = wrap ZL.nil
...
derive instance newtypeNat :: Newtype (Nat a) _
...
derive instance newtypeWrap :: Newtype (Wrap s a) _
...
answerUnsorted n = [before, self, after']
  where
    ...
    s = maybe "" fromCharArray $ ZL.head a
    local = Int.floor <<< toNumber $ fromInt (String.length s) - (end - target)
    { before, after } = String.splitAt local s
...
derive instance eqZMap :: (Eq c, Eq m) => Eq (ZMap c m)
derive instance newtypeZMap :: Newtype (ZMap k v) _
...
instance semiringZMap :: (Ord k, Semiring v) => Semiring (ZMap k v) where
  zero = wrap M.empty
  one = undefined
  add = over2 ZMap (M.unionWith add)
  mul = undefined

instance functorZMap :: Functor (ZMap k) where
  map f = over ZMap (map (map f))
...
instance semiringTrie :: (Ord c, Eq m, Semiring m) => Semiring (Trie c m) where
  ...
  add (Trie t1) (Trie t2) = Trie $ t1 + t2
